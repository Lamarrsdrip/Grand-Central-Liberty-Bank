...
export async function POST(request: NextRequest) {
  return handleApi(async () => {
    ...
    // Temporary debug — BUILD:2026-06-18T-bootstrap-admin-seed
    console.error("[login:debug] BUILD=2026-06-18T-bootstrap-admin-seed email=%s found=%s role=%s status=%s hashPrefix=%s",
      lookupEmail,
      user ? "yes" : "no",
      user?.role ?? "n/a",
      user?.status ?? "n/a",
